...
| Cylinder	     | Cylinder	               | <pre>result = Reference`<br>`result = { 'point': gom.Vec3d, 'radius': float, 'direction': gom.Vec3d, 'inner' : bool } # This does not work!</pre><br>⚠️ **Workaround:** set the result to <pre>{'default' : {'point': gom.Vec3d, 'radius': float, 'direction': gom.Vec3d, 'inner' : bool} }</pre> | Accepts any Cylinder Trait |
